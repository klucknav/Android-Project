...
/// The world state resulting from an updateUVs (@ref ownership "value type").
///
...
/// @ingroup trackable
/// Object types for heterogeneous query/updateUVs lists.
...
AR_DEFINE_ENUM(ArUpdateMode){
    /// @c updateUVs() will wait until a new camera image is available, or until
    ...
    /// @c updateUVs() will return the most recent ::ArFrame object.
    AR_UPDATE_MODE_BLOCKING = 0,
    /// @c updateUVs() will return immediately without blocking. If no new camera
    /// image is available, then @c updateUVs() will return the most recent
    /// ::ArFrame object.
...
/// blocking updateUVs is selected. This configuration is guaranteed to be
...
///
/// This call may updateUVs the pose of all created anchors and detected planes.
/// The set of updated objects is accessible through
/// ArFrame_getUpdatedTrackables().
///
/// @c updateUVs() in blocking mode (see ::ArUpdateMode) will wait until a
/// new camera image is available, or until the built-in timeout
/// (currently 66ms) is reached.
/// If the camera image does not arrive by the built-in timeout, then
/// @c updateUVs() will return the most recent ::ArFrame object. For some
...
/// not to change until updateUVs() is called.
///
/// @param[in]    session   The ARCore session.
...
/// updateUVs this estimate based on its understanding of the world. If the
...
/// updateUVs this estimate based on its understanding of the world. If an optional
